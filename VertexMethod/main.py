...
temperature_index = 2  # Выбираем значение температуры из b
...
if __name__ == "__main__":
    ...
    for i in range(N_mc):
        ...
        E_before = E(X_new_configuration, temperature_index)
        if E_before > E_max:
            EquateLists(X_new_configuration, X)
            Sum_estimators += Sum_estimator_for_mean_E(X_new_configuration, temperature_index)
            continue
        ...
        if delta_E > 0:
            ...
            if W >= rn.random():  # переходим или нет в новую конфигурацию
                ...
                Sum_estimators += Sum_estimator_for_mean_E(X, temperature_index)
            else:
                EquateLists(X_new_configuration, X)
                Sum_estimators += Sum_estimator_for_mean_E(X, temperature_index)
        else:
    ...
    Numerical_result = D / 2 * coth(b[temperature_index] / 2)
